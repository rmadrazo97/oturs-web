...
<div class="<?php echo $classes;?>" <?php echo implode( ' ', array_filter( $azpattributes ) ) ;?>>
	...
	?>
		...
		<?php if( isset($this->toStoreGlobalVar['columnitems']) && count( $this->toStoreGlobalVar['columnitems'] )): 
			foreach ( $this->toStoreGlobalVar['columnitems'] as $key => $col) : ?>
			<?php
			$colID = '';
			if(!empty($col['el_id'])){
				$colID .= 'id="'.$col['el_id'].'"';
